...
namespace SanteDB.Messaging.FHIR.Extensions.Patient
{
    ...
    public class EducationLevelExtension : IFhirExtensionHandler
    {
        ...
        {
            if(modelObject is Core.Model.Roles.Patient patient && patient.EducationLevelKey.HasValue)
            {
                yield return new Extension(this.Uri.ToString(), DataTypeConverter.ToFhirCodeableConcept(patient.EducationLevelKey ?? patient.EducationLevel?.Key));
            }
        }
        ...
        public bool Parse(Extension fhirExtension, IdentifiedData modelObject)
        {
            ...
            {
                patient.EducationLevel = DataTypeConverter.ToConcept(cc);
                return true;
            }
