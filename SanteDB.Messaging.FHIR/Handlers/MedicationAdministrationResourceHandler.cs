...
namespace SanteDB.Messaging.FHIR.Handlers
{
    ...
    public class MedicationAdministrationResourceHandler : RepositoryResourceHandlerBase<MedicationAdministration, SubstanceAdministration>
    {
        ...
        {
            return instance is Immunization || instance is SubstanceAdministration sbadm && ((!this.IZ_TYPES.Contains(sbadm.TypeConceptKey.GetValueOrDefault())) || sbadm.TypeConceptKey == DRUG_THERAPY);
        }
        ...
        protected override SubstanceAdministration MapToModel(MedicationAdministration resource)
        {
            ...
            if (null == retVal?.TypeConcept || !retVal.TypeConcept.Key.HasValue || !IZ_TYPES.Contains(retVal.TypeConcept.Key.Value))
            {
                retVal.TypeConcept = null;
  ...
  //          var typeReference = Expression.MakeBinary(ExpressionType.Equal, Expression.Convert(Expression.MakeMemberAccess(query.Parameters[0], typeof(SubstanceAdministration).GetProperty(nameof(SubstanceAdministration.TypeConceptKey))), typeof(Guid)), Expression.Constant(DRUG_THERAPY));
